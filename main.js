...
// 1. ยาเรียง A-Z และเพิ่ม Morphine IR
const medList = [
...
function handleFormSubmit(e) {
  ...
  if(hnVal.length !== 6 || isNaN(hnVal)) { Swal.fire('ข้อผิดพลาด', 'HN ต้องเป็นตัวเลข 6 หลัก', 'warning'); return; }
  ...
  // เพิ่มข้อมูลผู้ทำ ACP และวันที่
  ...
  const formData = {
    ...
    lab: { 
      ...
      date:document.getElementById('lab_date').value // เพิ่ม Lab Date
    },
...
function renderHistoryItems(list) {
  ...
  c.innerHTML = list.map(h => {
     ...
     // Lab + Date
     let labHtml = '';
     if(h.lab_cr||h.lab_egfr) {
        const ld = h.lab_date ? `<br><span class="text-muted small">วันที่ Lab: ${formatDateTH(h.lab_date)}</span>` : '';
        labHtml = `<div class="alert alert-light border p-2 mb-2 small"><i class="fas fa-flask text-danger"></i> <b>Lab:</b> Cr:${h.lab_cr||'-'} eGFR:${h.lab_egfr||'-'} ${ld}</div>`;
     }
     ...
     // ACP Detail
     let acpHtml = '';
     if(h.acp && (Object.keys(h.acp).length > 2 || h.acp.maker)) { // check if meaningful data
        acpHtml = `<div class="mt-2 small border-top pt-2 bg-light p-2 rounded"><i class="fas fa-file-contract"></i> <b>ACP:</b> `;
        if(h.acp.maker) acpHtml += `<div>ผู้ทำ: ${h.acp.maker} (${formatDateTH(h.acp.date)})</div>`;
        Object.entries(h.acp).forEach(([k,v])=>{
            if(k!=='maker' && k!=='date' && v!=='Undecided') acpHtml += `<span class="me-2 d-inline-block">• ${k}: <u>${v}</u></span>`;
        });
        acpHtml += `</div>`;
     }
     const v = h.vitals||{};
