...
// --- Helper: วันที่ภาษาไทย (Fix Date Shift) ---
// รับ string '2026-01-05' แล้วแสดงผลเลย ไม่ผ่าน new Date() เพื่อป้องกัน timezone shift
function formatDateTH(dateStr) {
  if (!dateStr) return '-';
  // ถ้า backend ส่งมาเป็น ISO string ยาวๆ ให้ตัดเอาแค่ 10 ตัวแรก
  const isoDate = dateStr.substring(0, 10); // ได้ '2026-01-05'
...
function renderActivePatients() {
  ...
  document.getElementById('activePatientList').innerHTML = f.length ? f.map(p => {
    ...
    <div class="col-md-4 col-sm-6">
      <div class="card p-3 shadow-sm patient-card ${typeClass}" onclick="editPatient('${p.hn}')">
         <div class="d-flex justify-content-between align-items-start">
            <div>
               <h5 class="mb-1 fw-bold text-dark">${p.name}</h5>
               <p class="mb-0 text-secondary small">HN: ${p.hn}</p>
            </div>
            <span class="status-badge">${p.type_admit}</span>
         </div>
         <hr class="my-2" style="opacity:0.1">
         <small class="text-muted"><i class="far fa-calendar-check"></i> นัดถัดไป: <span class="text-dark fw-bold">${formatDateTH(p.next_visit_date)}</span></small>
      </div>
    </div>`;
...
function renderApptList(s) {
  ...
  const p=allPatients.filter(x => x.next_visit_date && x.next_visit_date.substring(0,10) === s);
  l.innerHTML = p.length ? p.map(x => {
    const typeClass = getTypeClass(x.visit_type);
    return `
    <div class="col-md-6">
       <div class="card p-3 shadow-sm ${typeClass}" onclick="editPatient('${x.hn}')" style="cursor:pointer; border-left-width: 8px;">
          <div class="d-flex justify-content-between">
             <h5 class="mb-1 fw-bold">${x.name}</h5>
             <span class="badge bg-white text-dark border">${x.visit_type}</span>
          </div>
          <small class="text-secondary">HN: ${x.hn}</small>
       </div>
    </div>`;
...
function initSlider() { 
  const c=document.getElementById('dateSlider'); c.innerHTML=''; const d=new Date(); 
  const th=['อา','จ','อ','พ','พฤ','ศ','ส']; 
  for(let i=0;i<14;i++){ 
    const t=new Date(d); t.setDate(d.getDate()+i); 
    // Manual Format YYYY-MM-DD to avoid UTC shift
  ...
  renderApptList(`${y}-${m}-${day}`); 
}
// (ฟังก์ชันย่อยอื่นๆ คงเดิม เพื่อความกระชับ)
...
function showHistoryModal(){const hn=document.getElementById('hn').value,name=document.getElementById('fullname').value;if(!hn)return;const modal=new bootstrap.Modal(document.getElementById('historyModal'));document.getElementById('historyPatientName').innerText=name;document.getElementById('historyLoading').classList.remove('d-none');document.getElementById('historyContent').classList.add('d-none');modal.show();const l=document.getElementById('lat').value,lg=document.getElementById('long').value,b=document.getElementById('btnMapHistory');if(l&&lg){b.classList.remove('d-none');b.onclick=()=>window.open(`https://www.google.com/maps/dir/?api=1&destination=${l},${lg}`,'_blank');}else b.classList.add('d-none');fetch(SCRIPT_URL+'?op=getHistory&hn='+hn).then(r=>r.json()).then(d=>{renderHistoryItems(d);document.getElementById('historyLoading').classList.add('d-none');document.getElementById('historyContent').classList.remove('d-none');});}
function renderHistoryItems(l){const c=document.getElementById('historyContent');if(!l.length){c.innerHTML='<div class="alert alert-warning">ไม่พบประวัติ</div>';return;}c.innerHTML=l.map(h=>{const d=formatDateTH(h.date);let m='-';if(h.meds&&h.meds.length)m='<ul class="mb-0 ps-3 small text-muted">'+h.meds.map(x=>`<li>${x.name}</li>`).join('')+'</ul>';return `<div class="card mb-2 shadow-sm history-card"><div class="card-header bg-white d-flex justify-content-between"><span class="history-date">${d}</span><span>PPS: ${h.pps}%</span></div><div class="card-body"><div class="row"><div class="col-6 border-end"><small><b>Vitals:</b> ${h.vitals?.bp||'-'}</small><br><small><b>Plan:</b> ${h.plan||'-'}</small></div><div class="col-6"><small class="fw-bold">Meds:</small>${m}</div></div></div></div>`;}).join('');}
