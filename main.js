...
function openEditRegistration(hn) {
  ...
  showPage('register');
  Swal.fire({title: 'โหมดแก้ไข', text: `กำลังแก้ไขข้อมูลของ ${p.name}`, icon: 'info', toast: true, position: 'top-end', showConfirmButton: false, timer: 3000});
}
...
// ** แก้ไข getLocation: ไม่เด้ง Map แล้ว แค่บันทึกค่า **
function getLocation() { 
  if(navigator.geolocation){ 
    Swal.fire({title:'กำลังระบุพิกัด...',didOpen:()=>Swal.showLoading()}); 
    navigator.geolocation.getCurrentPosition(p=>{ 
      document.getElementById('lat').value=p.coords.latitude; 
      document.getElementById('long').value=p.coords.longitude; 
      updateMapBtnStatus(true); 
      Swal.fire({icon:'success', title:'บันทึกพิกัดสำเร็จ', text:`${p.coords.latitude.toFixed(5)}, ${p.coords.longitude.toFixed(5)}`, timer:1500, showConfirmButton:false});
    }, err=>{
